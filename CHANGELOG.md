...
- Every **data download link on every page was a 404**. `release_url` in
  ...
  release -- `bal-insitu-nrt-report/releases/download/v0.2.0` -- while the file names
  appended to it are the current ones, so the URLs missed on repository, tag and file name
  at once. It now points at `bal-report/releases/download/v0.5.0`, and every link a page
...
empty `content/_freeze/` and `content/docs/`: 15 pages and 83 figures in 196s, no chunk error and
...
- **The pressure pages are back**: `bo_pres.qmd`, `bo_cora_pres.qmd` and their `_qc`
  counterparts, with a **Pressure** menu restored to the navbar between Profile Summary and
  Temperature, and a **Pressure** group in the home page's Contents list. Each is its
...
- Page rendering no longer re-groups the summaries by profile. `netcdf_summary_1()` and the
  ...
  grouping was a no-op costing 15s and 6s per call. A whole-site render drops from 140s to
  110s, with every figure byte-identical and every page identical apart from
